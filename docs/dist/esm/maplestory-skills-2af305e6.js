...
const flush = () => {
    ...
    const timeout = plt.$flags$ & 2 /* queueAsync */
...
const attachStyles = (elm, cmpMeta, mode) => {
    const styleId = addStyle((elm.shadowRoot && supportsShadowDom)
        ? elm.shadowRoot
    ...
    if (!elm['s-al']) {
        ...
        if (!(hostRef.$flags$ & 512 /* hasLoadedComponent */)) {
            ...
            if (!ancestorComponent) {
                ...
                {
                    setTimeout(() => plt.$flags$ |= 2 /* queueAsync */, 999);
                }
            }
        }
    ...
    if ((hostRef.$flags$ & 256 /* hasInitializedComponent */) === 0) {
        ...
        if (!Cstr.isStyleRegistered && Cstr.style) {
            ...
            if (cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await import('./chunk-c152be33.js').then(m => m.scopeCss(style, scopeId, false));
            }
...
const patchBrowser = async () => {
    ...
    if (!window.customElements) {
        // @ts-ignore
        await import('./chunk-1aaed225.js');
    }
