...
export class DeathSentenceComponent {
  ...
  @Prop({ reflectToAttr: true }) private max: number = DeathSentence.maxLevel;

  getRequirements(): string[] {
    return [
      `Level ${ DeathSentence.levelRequirement[this.level] }+`,
  ...
  render() {
    return [
      ...
      <ms-skill-overlay heading={ DeathSentence.name }
                        element="Dark"
                        level={ this.level }
        ...
        <div slot="description">
          ...
          dealing <span>{ DeathSentence.values.damage[this.level] }%</span> dark damage each
