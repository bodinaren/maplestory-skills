...
import { BlinkValues } from "../../../../global/values/runeblade";
...
export class BlinkComponent {
  ...
  @Prop({ reflectToAttr: true }) level: number = BlinkValues.minLevel;

  @Prop({ reflectToAttr: true }) private max: number = BlinkValues.maxLevel;

  render() {
    return [
      <ms-icon name="blink"></ms-icon>,
      <ms-skill-overlay heading="Blink"
                        level={ this.level }
                        max={ this.max }>
