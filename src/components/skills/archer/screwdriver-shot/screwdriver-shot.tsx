...
export class ScrewdriverShotComponent {
  ...
  @Prop({ reflectToAttr: true }) private max: number = ScrewdriverShot.maxLevel;

  getRequirements(): string[] {
    return [
      `Level ${ ScrewdriverShot.levelRequirement[this.level] }+`,
      `Rapid Shot [Level 8+]`,
    ];
  }
  ...
  render() {
    return [
      ...
      <ms-skill-overlay heading={ ScrewdriverShot.name }
                        level={ this.level }
        ...
        <div slot="description">
          ...
          dealing <span>{ ScrewdriverShot.values.stage1[this.level] }%</span> damage to <span>20</span> enemies in its path.
          Hold the skill key down for <span>0.5</span> sec to enhance the arrow to <span>Lv. 2</span>,
          increasing its damage to <span>{ ScrewdriverShot.values.stage2[this.level] }%</span>.
          Hold the skill key down for <span>0.5</span> sec at <span>Lv. 2</span> to enhance it to <span>Lv. 3</span>,
          increasing its damage to <span>{ ScrewdriverShot.values.stage3[this.level] }%</span>.
