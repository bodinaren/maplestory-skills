...
export class EvasiveSalvoComponent {
  ...
  @Prop({ reflectToAttr: true }) private max: number = EvasiveSalvo.maxLevel;

  getRequirements(): string[] {
    return [
      `Level ${ EvasiveSalvo.levelRequirement[this.level] }+`,
    ];
  }
  ...
  render() {
    return [
      ...
      <ms-skill-overlay heading={ EvasiveSalvo.name }
                        level={ this.level }
        ...
        <div slot="description">
          ...
          deal <span>{ EvasiveSalvo.values.damage[this.level] }%</span> damage each to up to <span>5</span> enemies.
