...
export class ArrowStormComponent {
  ...
  @Prop({ reflectToAttr: true }) private max: number = ArrowStorm.maxLevel;

  getRequirements(): string[] {
    return [
      `Level ${ ArrowStorm.levelRequirement[this.level] }+`,
  ...
  render() {
    return [
      ...
      <ms-skill-overlay heading={ ArrowStorm.name }
                        level={ this.level }
        ...
        <div slot="description">
          ...
          deals <span>{ ArrowStorm.values.damage[this.level] }%</span> damage <span>20</span> times to <span>8</span> enemies
