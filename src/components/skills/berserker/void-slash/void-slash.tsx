...
export class VoidSlashComponent {
  ...
  @Prop({ reflectToAttr: true }) private max: number = VoidSlash.maxLevel;

  getRequirements(): string[] {
    return [
      `Level ${ VoidSlash.levelRequirement[this.level] }+`,
    ];
  }
  ...
  render() {
    return [
      ...
      <ms-skill-overlay heading={ VoidSlash.name }
                        element="Dark"
                        level={ this.level }
        ...
        <div slot="description">
          Swing your greatsword upward to deal <span>{ VoidSlash.values.firstDamage[this.level] }%</span> dark damage
          ...
          dealing <span>{ VoidSlash.values.secondDamage[this.level] }%</span> dark damage <span>2</span> times
